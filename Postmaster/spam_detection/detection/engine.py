...
Each method runs independently, then we combine their scores into a
weighted consensus. When multiple methods agree, we listen louder.
"""
...
def run_all(
    ...
    """
    logger.info("Running Fourier detection (Feel the Rhythm) ...")
    fourier_df = fourier.detect(df, sensitivity=sensitivity)

    logger.info("Running Matrix Profile detection (Feel the Rhyme) ...")
    mp_df = matrix_profile.detect(df, sensitivity=sensitivity)

    logger.info("Running Ensemble detection (Get on Up) ...")
    ensemble_df = ensemble.detect(df, sensitivity=sensitivity)

    logger.info("Running EWMA detection (It's Bobsled Time) ...")
    ewma_df = ewma.detect(df, sensitivity=sensitivity)

    # Start with the base data
    base_cols = ["Domain", "Date", "SpamRate"]
    if "DomainReputation" in df.columns:
        base_cols.append("DomainReputation")
    if "spam_rate_change" in df.columns:
        base_cols.append("spam_rate_change")
