"""EWMA (Exponentially Weighted Moving Average) Trend Analysis for email clicks.

Tracks the deviation of click count from its EWMA and classifies the
current trajectory as one of:
  - normal: within expected bands
  - accelerating: pulling away from the trend (engagement rising or falling)
  - decelerating: reverting toward the trend
  - breakout: extreme deviation from EWMA (potential emergency or contamination)

Plain-English: "Is this domain's click momentum abnormal right now?"
"""
...
def detect(
    ...
    """Run EWMA anomaly detection on each domain.
    ...
    for domain, grp in df.groupby("Domain"):
        ...
        ewma = pd.Series(clicks).ewm(span=span, adjust=False).mean().values

        # Deviation as % of EWMA
        deviation = (clicks - ewma) / np.where(ewma > 0, ewma, 1e-10)
